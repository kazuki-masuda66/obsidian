# Eagle-X-ONE-JP-session-memo-FY23-reviewとFY24-target

## 概要
ONE JP session memoに関するメモ。ONE Japan session宿題事項、FY23 BID Activity and review、Fukai-san Feedback、FY24 BID Target、Scope wise possible change、JPN dedicated case/solution、Required Process review、佐久間さん個別セッションについて詳細に記録されています。
...
- How to Save Tender Bid File and Documents?
  - Customer formatのときは事前にファイルとT&Cを保存するという目的を先に伝える
- How to Check the Status List?
  - How to Find a Folder in ONE Shared Drive?とセット
  - Pricerの提出状況も見えるということも伝える
- 逆に短すぎる、5分くらいでも大丈夫
- How to Contact Eagle X Support Team and CST?
  - どういうときに連絡する
...
## 次アクション
- [ ] 関連ノートにリンク（[[Eagle X]]、[[ONE JP session memo]]など）
- [ ] 必要に応じてMemory Noteに変換

-jp-session-memo`n#memory #work/one #work/one/eagle-x #work/one/dx